...
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*&      Form CALL_VA01_NEW_ORDERS_ORIGIN_PO
*&---------------------------------------------------------------------*
*       CALL TRANSACTION auf Transaktion VA01                          *
*       Include end customer pO info.                                  *
*&---------------------------------------------------------------------*
FORM call_va01_new_orders_origin_po USING rc.
  "|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
  "||||                                                             ||||
...
*  ENDLOOP.
  ...
  "/////////////////////////////////////////////////////////////////////
  "////
  "//// 표준 IDOC ORDERS ORDE 프로세스는 그대로 유지 ★★★★★
  "////
  "/////////////////////////////////////////////////////////////////////
...
*- Übersicht zweizeilig einspielen ------------------------------------*
*- call transaction double-line entry ---------------------------------*
  PERFORM va01_dynpro_uebersicht2.
*-Zusätzliche Daten für Dynpros vorsehen (Kundenerweiterungen)---------*
*-additional data's for new dynpro fields (customer exit)--------------*
  sy-subrc = 0.
  PERFORM customer_function_dynpro.
*- Auftragserfassung Einmalkunde --------------------------------------*
*- One-Time Customer Address Entry ------------------------------------*
  PERFORM dynpro_einmalkunde.
*-Zusätzliche Daten für Dynpros vorsehen (Kundenerweiterungen)---------*
*-additional data's for new dynpro fields (customer exit)--------------*
  sy-subrc = 0.
  PERFORM customer_function_dynpro.
...
*- call transaction header partner data -------------------------------*
  PERFORM va01_dynpro_kopf_partner.
...
*- call transaction header business data ------------------------------*
  PERFORM va01_dynpro_kopf_kaufmann.
*-Zusätzliche Daten für Dynpros vorsehen (Kundenerweiterungen)---------*
*-additional data's for new dynpro fields (customer exit)--------------*
  sy-subrc = 0.
  PERFORM customer_function_dynpro.
...
*- call transaction header business shipping data ---------------------*
  PERFORM va01_dynpro_kopf_kaufmann_kde2.
*-Zusätzliche Daten für Dynpros vorsehen (Kundenerweiterungen)---------*
*-additional data's for new dynpro fields (customer exit)--------------*
  sy-subrc = 0.
  PERFORM customer_function_dynpro.
...
*- call transaction header business invoice  data ---------------------*
  PERFORM va01_dynpro_kopf_kaufmann_kde3.
*-Zusätzliche Daten für Dynpros vorsehen (Kundenerweiterungen)---------*
*-additional data's for new dynpro fields (customer exit)--------------*
  sy-subrc = 0.
  PERFORM customer_function_dynpro.
...
*- call transaction header purchase order data ------------------------*
  PERFORM va01_dynpro_kopf_bestell.
*-Zusätzliche Daten für Dynpros vorsehen (Kundenerweiterungen)---------*
*-additional data's for new dynpro fields (customer exit)--------------*
  sy-subrc = 0.
  PERFORM customer_function_dynpro.
...
*- call transaction header conditions ---------------------------------*
  PERFORM va01_dynpro_kopf_kondition.
*-Zusätzliche Daten für Dynpros vorsehen (Kundenerweiterungen)---------*
*-additional data's for new dynpro fields (customer exit)--------------*
  sy-subrc = 0.
  PERFORM customer_function_dynpro.
*- Kopf Zahlungskarten einspielen -------------------------------------*
*- call transaction header payment cards ------------------------------*
  PERFORM va01_dynpro_kopf_krpl.
*-Zusätzliche Daten für Dynpros vorsehen (Kundenerweiterungen)---------*
*-additional data's for new dynpro fields (customer exit)--------------*
  sy-subrc = 0.
  PERFORM customer_function_dynpro.
...
*- determine input method ---------------------------------------------*
  ...
  IF input_method = 'X'.
    EXPORT lt_bdcdata TO MEMORY ID 'idoc_test_bdcdata'.
  ELSE.
